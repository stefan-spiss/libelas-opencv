...
Modified by Stefan Spiss, 2020
Changes made:
    - Added OpenCV to load images from file and visualize the input images and the resulting disparity images.
    - Refactored the output file name generation to mordern C++.
...
void process (const char* file_1,const char* file_2) {
  ...
  cv::Mat imgLeft = cv::imread(file_1, cv::IMREAD_GRAYSCALE);
  cv::Mat imgRight = cv::imread(file_2, cv::IMREAD_GRAYSCALE);
  ...
  I1 = imageFromCvMat(imgLeft);
  I2 = imageFromCvMat(imgRight);
  ...
  /* cv::Mat imgLeft = cvMatFromImage(I1); */
  /* cv::Mat imgRight = cvMatFromImage(I2); */
  ...
  cv::Mat dispLeft = cvMatFromImage(D1);
  cv::Mat dispRight = cvMatFromImage(D2);

  cv::imshow("leftImg", imgLeft);
  cv::imshow("rightImg", imgRight);
  cv::imshow("leftDisp", dispLeft);
  cv::imshow("rightDisp", dispRight);
  ...
  std::string inputStr1(file_1);
  std::string inputStr2(file_2);
  
  std::string output1 = inputStr1.substr(0, inputStr1.length() - 4) + "_disp.pgm";
  std::string output2 = inputStr2.substr(0, inputStr2.length() - 4) + "_disp.pgm";
  savePGM(D1,output1.c_str());
  savePGM(D2,output2.c_str());
