...
/*  Changes made (2020):                                                     */
/*      - Removed all compile warnings. For that unused variables were       */
/*        commented/removed, printf types were adapted (%llx instead of %lx  */
/*        for unsigned long long types), initialization  of variables was    */
/*        added and static_casts were added.                                 */
...
/* Modified by Stefan Spiss, 2020:                                            */
/*      - For printing unsigned long long, the printf parameter was changed  */
/*        to %llx instead of %lx.                                            */
              ...
              int firstitemcount, int alignment)
{
  ...
  if (static_cast<size_t>(alignment) > sizeof(int *)) {
...
void initializetrisubpools(struct mesh *m, struct behavior *b)
{
  ...
  if ((b->voronoi || b->neighbors) &&
      (static_cast<size_t>(trisize) < 6 * sizeof(triangle) + sizeof(int))) {
...
{
  /* Modified by Stefan Spiss, 2020:                                 */
  /*      - Added since triflaws is never used.                     */
  (void) triflaws; // never used
  struct otri horiz;
  struct otri top;
  struct otri botleft, botright;
  struct otri topleft, topright;
                         ...
                         vertex endpoint2)
{
  ...
  /* Modified by Stefan Spiss, 2020:                                 */
  /*      - Added since collinear only set but never used.          */
  (void) collinear; // set below, but not used otherwise
  float ex, ey;
  float tx, ty;
  float etx, ety;
  float split, denom;
