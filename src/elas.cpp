...
Changes made:
    ...
    - Refactored for loops in which the size of a collection is used to set the iteration limit to use auto i=0u instead
        of int32_t i=0.
    - Changed free in line 1567 to delete[] since array vals is initialized with new (fixed warning).
...
void Elas::addCornerSupportPoints(vector<support_pt> &p_support) {
  ...
  // find closest d
  for (auto i=0u; i<p_border.size(); i++) {
    int32_t best_dist = 10000000;
    for (auto j=0u; j<p_support.size(); j++) {
...
vector<Elas::triangle> Elas::computeDelaunayTriangulation (vector<support_pt> p_support,int32_t right_image) {
  ...
  k=0;
  if (!right_image) {
    for (auto i=0u; i<p_support.size(); i++) {
...
void Elas::computeDisparityPlanes (vector<support_pt> p_support,vector<triangle> &tri,int32_t right_image) {
  (void) right_image; // never used
  // init matrices
  Matrix A(3,3);
  Matrix b(3,1);
  
  // for all triangles do
  for (auto i=0u; i<tri.size(); i++) {
